...
async function main() {
  ...
  if (!key) throw new Error('Usage: node scripts/check-agent-exec.cjs <logged-in-codex-home> <host> <user> <absolute-key>')
  await mkdir(resolve('artifacts'), { recursive: true })
  const directory = await mkdtemp(resolve('artifacts/agent-exec-'))
  ...
  try {
    ...
    await runtime.request('turn/start', { threadId: thread.id, input: [{ type: 'text', text: '查看 home-server 当前 TCP 和 UDP 监听端口及对应进程，再查看运行失败的 systemd 服务。只读查询。', text_elements: [] }] })
    const turn = await completed
    if (turn.error) throw new Error(turn.error.message)
    const data = await store.read()
    assert.ok(calls.includes('servers_exec'))
    assert.equal(data.servers.length, 1)
    assert.ok(data.history.some(item => item.title === '远程命令结束'))
    assert.ok(replies.length > 0)
    await writeFile(join(directory, 'reply.json'), JSON.stringify({ calls, replies }, null, 2))
    console.log(JSON.stringify({ passed: true, calls, replies, storage: 'isolated inventory' }))
